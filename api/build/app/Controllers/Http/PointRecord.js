...
class PointRecord {
    constructor() {
        this.monthFreedom = async (id_empresa, id_pdf, mes) => {
            ...
            `);
            return liberacaoPdf?.rows.length > 0 ? liberacaoPdf?.rows[0] : [];
        };
    }
    ...
    async pointRecordGenerator({ request, response, auth, }) {
        try {
            ...
            const monthReleased = await this.monthFreedom(auth.user?.id_empresa, 1, competencia);
            if (!monthReleased?.dt_inicial || !monthReleased?.dt_final) {
                return response.badRequest({
                    error: "Empresa não liberou para gerar o recibo",
                });
            }
            let dateRequestInitial = luxon_1.DateTime.fromISO(new Date(`${monthReleased.dt_inicial}`)
                .toISOString()
                .replace(".000Z", ""))
                .toFormat("dd-LL-yyyy")
                .toString();
            let dateRequestFinish = luxon_1.DateTime.fromISO(new Date(`${monthReleased.dt_final}`).toISOString().replace(".000Z", ""))
                .toFormat("dd-LL-yyyy")
                .toString();
...
exports.default = PointRecord;
//# sourceMappingURL=PointRecord.js.map