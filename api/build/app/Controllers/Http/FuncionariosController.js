...
class FuncionariosController {
    async create({ request, response }) {
        try {
            await request.validate({ schema: Validator_1.schema.create(Funcionario_2.FuncionarioSchemaInsert) });
    ...
    async getById({ request, response }) {
        ...
        if (id_funcionario) {
            const funcionario = await Database_1.default.connection('pg').query()
            ...
            if (funcionario[0]) {
                let funcionario_erp = await Database_1.default.connection('oracle')
                    .rawQuery(`
    ...
    async addArea({ request, response }) {
        ...
        if (id_funcionario && arrayArea) {
            let deleta = await FuncionarioArea_1.default.query().where('id_funcionario', '=', id_funcionario).delete();
            if (deleta) {
            ...
            await FuncionarioArea_1.default.query().where('id_funcionario', '=', id_funcionario).delete();
    ...
    async removeArea({ request, response }) {
        ...
        await Promise.all(arrayArea.forEach(async (element) => {
            let areaFunc = await FuncionarioArea_1.default.query().select('*').where('id_funcionario', '=', id_funcionario).andWhere('id_area', '=', element).limit(1);
    ...
    async getDepartamentsbyFuncionario(id_funcionario) {
        const areas = await FuncionarioArea_1.default.query().select('');
    }
    async updateProfile({ request, response, auth }) {
        try {
            await request.validate({ schema: Validator_1.schema.create(Funcionario_2.updateProfileFuncionario) });
    ...
    async checkByCpf({ request, response }) {
        await request.validate({ schema: Validator_1.schema.create({ cpf: Validator_1.schema.string(), id_empresa: Validator_1.schema.number() }) });
        let dados = request.body();
    ...
    async EventsReceiptFormByFuncionario({ request, auth, response }) {
        try {
            ...
            if (dados.data) {
                let funcionario = await Funcionario_1.default.findBy('id_funcionario', auth.user?.id_funcionario);
                let query = await Database_1.default
                    .connection('oracle')
                    .rawQuery(`
                                    SELECT DISTINCT 
    ...
    async deleteAccount({ request, response }) {
        try {
            await request.validate({ schema: Validator_1.schema.create({ id_funcionario: Validator_1.schema.number() }) });
            let dados = request.body();
    ...
    async dotCardPdf({ request, response, auth }) {
        try {
            ...
            if (dados.data) {
                ...
                let confirmacao = await ConfirmarPdf_1.default.query().select('*').where('id_funcionario', '=', `${funcionario?.id_funcionario}`).andWhere('data_pdf', '=', `${dados.data}`);
                let file = await (0, S3_1.uploadPdfEmpresa)(pdfTemp.filename, auth.user?.id_empresa);
                if (file) {
                    fs_1.default.unlink(pdfTemp.filename, () => { });
                    response.json({
    ...
    async getVideos({ response, auth }) {
        try {
            let dados = await Database_1.default.connection('pg')
                .rawQuery(`
