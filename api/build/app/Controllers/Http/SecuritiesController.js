...
class SecuritiesController {
    async createGroup({ request, response }) {
        ...
        await SecurityGroup_1.default.create({
            nome: dados.nome,
            itens: dados.itens,
    ...
    async getGroupById({ request, auth }) {
        await request.validate({
            schema: Validator_1.schema.create({ id_grupo: Validator_1.schema.number() }),
        });
        let dados = request.body();
        if (auth.user)
    ...
    async createGroupUser({ request, response }) {
        await request.validate({
            schema: Validator_1.schema.create({
                id_grupo: Validator_1.schema.number(),
    ...
    async getGroupUserByIdUser({ response, auth }) {
        let query = await Database_1.default.rawQuery(`
            SELECT
            ug.id_usuario_grupo,
            g.nome as nome,
