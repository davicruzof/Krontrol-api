    ...
    try {
      ...
      if (dados.data) {
        ...
        let query = await Database.connection("oracle").rawQuery(`
                                    ...
                                    NVL(F.ENTRADA, '_____') AS ENTRADA,
                                    NVL(F.I_INI, '_____') AS I_INI,
                                    NVL(F.I_FIM, '_____') AS I_FIM,
                                    NVL(F.SAIDA, '_____') AS SAIDA,
                                    NVL(F.LINHA, '_____') AS LINHA,
                                    NVL(F.TABELA, '_____') AS TABELA,
                                    F.CODOCORR,
                                    NVL(F.NORMAL, '_____') AS NORMAL,
                                    NVL(F.EXTRA, '_____') AS EXTRA,
                                    NVL(F.OUTRA, '_____') AS OUTRA,
                                    NVL(F.A_NOT, '_____') AS A_NOT,
                                    NVL(F.BD_DEBITO, '_____') AS BD_DEBITO,
                                    NVL(F.BH_CREDITO, '_____') AS BH_CREDITO,
