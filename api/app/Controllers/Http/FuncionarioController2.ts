  ...
  public async EventsReceiptFormByFuncionario({
    ...
    try {
      ...
      if (dados.data) {
        ...
        if (!funcionario) {
          return response.badRequest({ error: "funcionário invalido" });
        }
        ...
        const liberacaoPdf = await Database.connection("pg").rawQuery(
          `SELECT * FROM public.vw_ml_flp_liberacao_recibos 
            ...
            AND empresa_id = ${funcionario?.id_empresa}
            `
        );
        ...
        let mes_liberado: any = new Date(
          `01/${liberacaoPdf.rows[0].mes_liberado}`
        )
          .toISOString()
          .replace(".000Z", "");
        let data_recebida: any = new Date(
          `01/${dados.data.split("-").reverse().join("/")}`
        )
          .toISOString()
          .replace(".000Z", "");
        ...
        const isMenorOuIgual = data_recebida <= mes_liberado;
