...
export default class SecuritiesController {
    ...
    public async createGroup({request,response}:HttpContextContract){
        ...
        await SecurityGroup.create({nome : dados.nome, itens : dados.itens, id_empresa : dados.id_empresa});
        response.json({sucess : "Criado com sucesso"});

    }


    public async getGroupById({request,response,auth}:HttpContextContract){

        await request.validate({ schema: schema.create({ id_grupo : schema.number()})});
        let dados = request.body();
        return await SecurityGroup.query().select('*').where('id_grupo',dados.id_grupo).where('id_empresa',auth.user?.id_empresa);
    }

    public async getAllGroups({response,auth}:HttpContextContract){
    ...
    public async updateGroup({request,response}:HttpContextContract){
        ...
        await request.validate({ schema: schema.create({ id_grupo : schema.number(), nome : schema.string.nullableAndOptional(), itens: schema.string() })});
        let dados = request.body();
    ...
    public async createGroupUser({request,response,auth}:HttpContextContract){
    ...
    public async getGroupUserByIdUser({request,response,auth}:HttpContextContract){
    ...
    public async updateGroupUser({request,response}:HttpContextContract){
        ...
        let dados = request.body();
        let groupUser = await SecurityGroupUser.findBy('id_usuario_grupo',dados.id_usuario_grupo);
