...
export default class PointRecord {
  ...
  private monthFreedom = async (id_empresa, id_pdf, mes) => {
  ...
  public async pointRecordGenerator({
    request,
    response,
    auth,
  }: HttpContextContract) {
    try {
      ...
      const monthReleased = await this.monthFreedom(
      ...
      let dateRequestInitial = DateTime.fromISO(
        new Date(`${monthReleased.dt_inicial}`)
          .toISOString()
          .replace(".000Z", "")
      )
        .toFormat("dd-LL-yyyy")
      ...
      let dateRequestFinish = DateTime.fromISO(
        new Date(`${monthReleased.dt_final}`).toISOString().replace(".000Z", "")
      )
        .toFormat("dd-LL-yyyy")
        .toString();
